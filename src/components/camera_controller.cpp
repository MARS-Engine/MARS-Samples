...
void camera_controller::update() {
    ...
    angle.x(angle.x() + get_input()->mouse_relative().y() * look_speed * get_delta_time());
    angle.y(angle.y() + get_input()->mouse_relative().x() * look_speed * get_delta_time());
    ...
    transform().set_position(transform().position() + transform().rotation() * (movement * get_delta_time()));
